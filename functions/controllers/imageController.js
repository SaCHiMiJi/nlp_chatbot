 ...
 */
const processLineImage = async (messageId, userId) => {
  try {
    ...
    if (!imageBuffer) {
      logger.error("Failed to retrieve image from LINE");
      return {
        success: false,
    ...
    try {
      await storageService.uploadImage(imageBuffer, userId);
    } catch (error) {
 ...
 */
const processDirectImage = async (imageBuffer) => {
  try {
    ...
    // Parse the result
    const result = typeof analysisJson === 'string' 
    ...
    logger.error("Error processing direct image:", error);
    return {
      success: false,
      error: error.message,
