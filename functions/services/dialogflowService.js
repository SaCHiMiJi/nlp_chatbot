 ...
 */
const formatDialogflowResponse = (text, messages = []) => {
  const response = {
    fulfillmentMessages: [
      {
 ...
 */
const handleDialogflowWebhook = async (request) => {
  try {
    logger.info("Received Dialogflow webhook request", {
    ...
    logger.error("Error handling Dialogflow webhook:", error);
